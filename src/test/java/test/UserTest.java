...
public class UserTest extends BaseTest {
    ...
    public void task06_fullFlow() {
        ...
        String id = createUser("E2E User", email);              // 1) yarat

        given().spec(publicSpec()).pathParam("id", id)       // 2) yoxla
                .when().get("/users/{id}")
                .then().statusCode(200).body("name", equalTo("E2E User"));

        given().spec(authSpec(adminToken)).pathParam("id", id) // 3) yenilə
                .body("{ \"name\": \"E2E Updated\", \"email\": \"" + email + "\" }")
                .when().put("/users/{id}")
                .then().statusCode(200);

        given().spec(publicSpec()).pathParam("id", id)       // 4) yenidən yoxla
                .when().get("/users/{id}")
                .then().statusCode(200).body("name", equalTo("E2E Updated"));

        given().spec(authSpec(adminToken)).pathParam("id", id) // 5) sil
                .when().delete("/users/{id}")
                .then().statusCode(anyOf(is(200), is(204)));

        given().spec(publicSpec()).pathParam("id", id)       // 6) silindiyini təsdiqlə
                .when().get("/users/{id}")
                .then().statusCode(404);
    }
}